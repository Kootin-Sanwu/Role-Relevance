...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../../frontend/views/job_roles.php');
    exit;
}
...
if (empty($organizationName) || empty($jobTitles)) {
    header("Location: ../../frontend/views/job_roles.php?msg=" . urlencode('Organization name and at least one job title are required.'));
}

// Check if the Organization ID exists in the session
if (!isset($_SESSION['OrganizationID'])) {
    header("Location: ../../frontend/views/job_roles.php?msg=" . urlencode('No organization found in session. Please log in again.'));
}
...
try {
    ...
    header("Location: ../../frontend/index.php?msg=" . urlencode('Job roles submitted successfully.'));
    exit();
