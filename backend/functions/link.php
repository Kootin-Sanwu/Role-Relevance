...
function generateApprovalLink($organizationEmail, $hashedPassword, $organizationName, $jobTitles, $organizationDescription)
{
    ...
    return "http://localhost/Local-Job-Role-Relevance-1/frontend/views/approval.php?action=approve&token=$token";
}
...
function generateDeclineLink($organizationEmail)
{
    ...
    return "http://localhost/Local-Job-Role-Relevance-1/frontend/views/approval.php?action=decline&token=$token";
}